...
Learn how Teleport's architecture leverages the Cosmos SDK Proof-of-Stake functionality, EVM compatibility and fast-finality from Tendermint Core's BFT consensus. {synopsis}

## Cosmos SDK

Teleport enables the full composability and modularity of the [Cosmos SDK](https://docs.cosmos.network/).
...
engines of other blockchains. Teleport is such an example of an ABCI application
...
Teleport enables EVM compatibility by implementing various components that together support all the EVM state transitions while ensuring the same developer experience as Ethereum:
...
Check the available Teleport [resources](./resources.md) {hide}
