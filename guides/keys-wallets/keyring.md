...
You can use `teleport keys` for help with the keys command and `teleport keys [command] --help` for more information about a particular subcommand.
...
```bash
teleport keys add mykey --keyring-backend test

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(teleport keys show mykey -a --keyring-backend test)
```
...
yes $KEYPASSWD | teleport keys add me
yes $KEYPASSWD | teleport keys show me
# start teleport with keyring-backend flag
teleport --keyring-backend=file start
```
