...
For example, the chain identifier can be changed to `teleport_9000-1` from a blank name by using:

```bash
teleport config "chain-id" teleport_9000-1
teleport config
{
...
After the necessary changes are made in the `client.toml`, then save. For example, if we directly change the chain-id from `teleport_9000-1` to `teleporttest_9000-1`, and output to number, it would change instantly as shown below.

```bash
teleport config
{
 "chain-id": "teleporttest_9000-1",
 "keyring-backend": "os",
 "output": "number",
 "node": "tcp://localhost:26657",
 "broadcast-mode": "sync"
